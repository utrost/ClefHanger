...
test('ships a mobile-first PWA shell for ClefHanger', () => {
  ...
  assert.match(html, /src="\.\/src\/app\.js\?v=clefhanger-slice36-android-recorder-2026-09-01"/);
  assert.match(html, /navigator\.serviceWorker\s*\.register\('\.\/sw\.js\?v=clefhanger-slice36-android-recorder-2026-09-01'\)/);
  ...
  assert.match(html, /Slice 36: Android recorder nudge \+ vocal gain/);
});
...
test('manifest and service worker describe an installable subpath-safe app shell', () => {
  ...
  const sw = read('sw.js');
  assert.match(sw, /clefhanger-pwa-v30/);
