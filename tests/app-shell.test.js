...
test('ships a mobile-first PWA shell for ClefHanger', () => {
  ...
  assert.match(html, /src="\.\/src\/app\.js\?v=clefhanger-slice55-mic-scoring-feedback-2026-09-10"/);
  assert.match(html, /navigator\.serviceWorker\s*\.register\('\.\/sw\.js\?v=clefhanger-slice55-mic-scoring-feedback-2026-09-10'\)/);
  ...
  assert.match(html, /data-app-version="clefhanger-slice55-mic-scoring-feedback/);
  assert.match(html, /Slice 55: mic scoring feedback/);
});
...
test('manifest and service worker describe an installable subpath-safe app shell', () => {
  ...
  const sw = read('sw.js');
  assert.match(sw, /clefhanger-pwa-v49/);
