...
test('ships a mobile-first PWA shell for ClefHanger', () => {
  ...
  assert.match(html, /src="\.\/src\/app\.js\?v=clefhanger-slice51-mic-octave-match-toggle-2026-09-02"/);
  assert.match(html, /navigator\.serviceWorker\s*\.register\('\.\/sw\.js\?v=clefhanger-slice51-mic-octave-match-toggle-2026-09-02'\)/);
  ...
  assert.match(html, /Slice 51: mic octave match toggle/);
});
...
test('manifest and service worker describe an installable subpath-safe app shell', () => {
  ...
  const sw = read('sw.js');
  assert.match(sw, /clefhanger-pwa-v45/);
