...
test('ships a mobile-first PWA shell for ClefHanger', () => {
  ...
  assert.match(html, /src="\.\/src\/app\.js\?v=clefhanger-slice44-content-catalog-2026-09-01"/);
  assert.match(html, /navigator\.serviceWorker\s*\.register\('\.\/sw\.js\?v=clefhanger-slice44-content-catalog-2026-09-01'\)/);
  ...
  assert.match(html, /Slice 44: content catalog/);
});
...
test('manifest and service worker describe an installable subpath-safe app shell', () => {
  ...
  const sw = read('sw.js');
  assert.match(sw, /clefhanger-pwa-v38/);
  for (const asset of ['./', './index.html', './manifest.webmanifest', './src/app.js', './src/core/audio.js', './src/core/game.js', './src/core/content.js', './src/core/pitch.js', './src/core/mic-diagnostics.js', './src/core/learning.js', './src/core/music-theory.js', './src/ui/staff-renderer.js', './icons/icon-192.svg', './icons/icon-512.svg', './icons/icon-192.png', './icons/icon-512.png']) {
