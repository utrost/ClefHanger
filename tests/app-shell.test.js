...
test('ships a mobile-first PWA shell for ClefHanger', () => {
  ...
  assert.match(html, /src="\.\/src\/app\.js\?v=clefhanger-slice34-quiet-mic-2026-09-01"/);
  assert.match(html, /navigator\.serviceWorker\s*\.register\('\.\/sw\.js\?v=clefhanger-slice34-quiet-mic-2026-09-01'\)/);
  ...
  assert.match(html, /data-app-version="clefhanger-slice34-quiet-mic/);
  assert.match(html, /Slice 34: quieter mic levels \+ raw instrument mic/);
});
...
test('manifest and service worker describe an installable subpath-safe app shell', () => {
  ...
  const sw = read('sw.js');
  assert.match(sw, /clefhanger-pwa-v28/);
