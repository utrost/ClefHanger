...
- In Settings, tap **Grant mic**. The settings calibration line should immediately show `Requesting mic…`; if Chrome never returns a permission result it should time out with a site-settings hint instead of appearing dead. If the browser prompts, allow microphone access; the status should change from mic-off to listening.
...
- Verify the HTML contains `clefhanger-slice15-chrome-mic-diagnostics`.
