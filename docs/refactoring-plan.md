...
**Current baseline:** the app is a dependency-free static PWA using native ES modules. Local checks are green at the time this plan was written (`npm run check`: 78 tests passing). The main architecture risk is not missing tests; it is continued feature work accumulating in `src/app.js` and broadening `src/core/game.js`.
